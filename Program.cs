...
app.MapGet("/currencyConversion/{date?}", async ([FromBody] CurrencyInput currencyInput, string? date , IConfiguration config) =>  
{
    ...
    DateTime passedDateTime = DateTime.Now;

    if (date != null && DateTime.TryParse(date, out passedDateTime) && passedDateTime.Date < DateTime.Now.Date)
        query["date"] = $"{passedDateTime.ToString("yyyy-MM-dd")}";
